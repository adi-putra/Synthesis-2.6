...
<body>
	...
	//if group id exist
	if (isset($hostid)) {
		//get all esxi versions and count
		$sysver = array();
		$params = array(
			"output" => array("lastvalue"),
			"hostids" => $hostid, //find by hostid
			"search" => array("name" => "esxi version")
		);
		...
		$vervalues = array_count_values($sysver); //group and count esxi versions
	}
	else {
		//get all esxi versions and count
		$sysver = array();
