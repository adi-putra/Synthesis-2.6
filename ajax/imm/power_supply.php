    ...
    <tbody>
      ...
      foreach ($hostid as $hostID) {
        ...
        foreach ($result as $item) {
          if (stripos($item["name"], "power system") !== false) {
            continue;
          }
          else {
            $psustatus = $item["lastvalue"];
          $psuname = substr($item["name"], 0, -20);
          print "<td><a href='hostdetails_imm.php?hostid=".$hostID."' target='_blank'>$gethostname</a></td>";
          print "<td>$psuname</td>";
          if ($item["error"] !== "") {
            ...
            if (is_numeric($psustatus)) {
                  ...
                  if ($psustatus == "Normal") {
                    ...
                    print "<td><button class='btn btn-block btn-danger btn-sm'>$psustatus</button></td></tr>";
                  } 
                }
            }
          }
