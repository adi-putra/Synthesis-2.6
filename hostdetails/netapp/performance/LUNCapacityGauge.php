                            ...
                            foreach ($LUN as $row) {
                                ...
                                if (strpos($name, 'LUN Available Capacity') !== false) {
                                    ...
                                    if(strpos($name, 'Backup') == false && strpos($name, 'Rescan') == false ){
                                        ...
                                        $name = explode('/', $name);
                                        $name = $name[1];
                                        $available_capacity = number_format($row["lastvalue"]/(1024*1024*1024*1024),2); //TB

                                        ?>
                                        setInterval(function() {
                                            hand_<?= $name; ?>.showValue(<?= $available_capacity ?>, am4core.ease.cubicOut);

                                            var available_capacity = <?= $available_capacity ?>;
                                            label_<?= $name; ?>.text = available_capacity.toString()+" TB";
                                       ...
                                       //lable and legend
                                        ...
                                        label_<?= $name; ?>.background.stroke = chart_<?= $name; ?>.colors.getIndex(10);
                                        label_<?= $name; ?>.fill = chart_<?= $name; ?>.colors.getIndex(10);
                                        label_<?= $name; ?>.text = "0";
                                ...
                                foreach ($LUN as $row) {
                                    ...
                                    if (strpos($name, 'LUN Total Capacity') !== false) {
                                        ...
                                        if(strpos($name, 'Backup') == false && strpos($name, 'Rescan') == false ){
                                            ...
                                            $name = $name[1];
                                ?>
                                <td><div  id="chartdiv_<?= $name; ?>" style="width: 90%;height: 300px;margin-left:10%"></div></td>
                                <?php
                                }}}
                                ?>
                            </tr>
