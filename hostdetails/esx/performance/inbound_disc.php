...
foreach ($hostid as $hostID) {
    ...
    foreach ($result as $host) {
        ...
        "hostids" => $hostID,
        "search" => array("name" => "inbound packets discarded"), //seach id contains specific word
        );
        //call api method
...
foreach ($itemid as $item) {
    ...
    if (empty($itemdata)) {
      ...
      $itemdata = json_encode($itemdata);
      $inbounddiscSeries .= 'createSeries("value", "' .$item["hostname"].": ".$item["name"]. '", '.$itemdata.');';
      $itemdata = array();
    }
}
...
//print $inbounddiscSeries;

if ($inbounddiscSeries == "") {
  print '<h5 style="color: red;">No data available</h5>';
}
else {
  print '<div id="inbounddiscchart" style="width: auto;height: 400px;"></div>
...
var inbounddisc_chart = am4core.create("inbounddiscchart", am4charts.XYChart);


inbounddisc_chart.numberFormatter.numberFormat = "#.##";
//inbounddisc_chart.fontSize = 12;

var title = inbounddisc_chart.titles.create();
title.text = 'Inbound packets discarded';
title.fontSize = 25;
...
//inbounddisc_chart.legend.fontSize = 12;
