...
foreach ($hostid as $hostID) {
    ...
    foreach ($result as $host) {
        ...
        "hostids" => $hostID,
        "search" => array("name" => "inbound packets with error"), //seach id contains specific word
        );
        //call api method
...
foreach ($itemid as $item) {
    ...
    if (empty($itemdata)) {
      ...
      $itemdata = json_encode($itemdata);
      $inbounderrorSeries .= 'createSeries("value", "' .$item["hostname"].": ".$item["name"]. '", '.$itemdata.');';
      $itemdata = array();
    }
}
...
//print $inbounderrorSeries;

if ($inbounderrorSeries == "") {
  print '<h5 style="color: red;">No data available</h5>';
}
else {
  print '<div id="inbounderrorchart" style="width: auto;height: 400px;"></div>
...
var inbounderror_chart = am4core.create("inbounderrorchart", am4charts.XYChart);


inbounderror_chart.numberFormatter.numberFormat = "#.##";
//inbounderror_chart.fontSize = 12;

var title = inbounderror_chart.titles.create();
title.text = 'Inbound packets with errors';
title.fontSize = 25;
...
//inbounderror_chart.legend.fontSize = 12;
