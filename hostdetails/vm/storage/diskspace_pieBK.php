...
<body>
  ...
  $params = array(
    ...
    "hostids" => $hostid,
    "search" => array("name" => array("Total disk space on", "Used disk space on")), //seach id contains specific word
    "searchByAny" => true
  );
  //call api method
  ...
  for ($i = 0; $i < $length; $i++) {
    ...
    $result = $zbx->call('item.get', $params);
    $total =0;
    if(stripos($result[0]['name'], "VMware: Total disk space on C:") !== false){

       $total = round($result[0]["lastvalue"]/1024/1024/1024, 2);
    ...
    foreach ($result as $disk) {
      ...
      if (stripos($disk["name"], "VMware: Used disk space on C:") !== false) {
        ...
        if ($disk["lastvalue"] == 0) {
          ...
          $usedvalue = round($disk["lastvalue"]/1024/1024/1024, 2);
      ...
      if(stripos($disk["name"], "VMware: Total disk space on C:") !== false){

        $totalValue = round($disk["lastvalue"]/1024/1024/1024, 2);
