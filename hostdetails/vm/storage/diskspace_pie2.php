...
<body>
  ...
  $params = array(
    ...
    "hostids" => $hostid,
    "search" => array("name" => array("VMware: Free disk space on", "(percentage)")), //seach id contains specific word
  //  "searchByAny" => true
  );
  //call api method
  ...
  for ($i = 0; $i < $length; $i++) {
    ...
    foreach ($result as $disk) {
      ...
      if (stripos($item["name"], "Free disk space") !== false) {
        if ($disk["lastvalue"] == 0) {
          $freevalue = 100;
          $usedvalue = 0;
        }
   ...
   for ($i = 0; $i < $length; $i++) {
    if (!empty($itemerror[$i])) {
      ...
      var diskspace_pie' . $i . ' = am4core.create("diskspace_pie' . $i . '", am4charts.PieChart);

      // Add dummy data and do not display value 
      diskspace_pie' . $i . '.data = [{
        "disabled": true,
        "empty": "Empty",
        "value": "100" 
      }];
      ...
      var title = diskspace_pie' . $i . '.titles.create();
      ...
      var pieSeries = diskspace_pie' . $i . '.series.push(new am4charts.PieSeries());
      ...
      diskspace_pie' . $i . '.hiddenState.properties.radius = am4core.percent(0);
      </script>';*/
      ...
      print '<!-- Chart code -->
          ...
          // Add data
          diskspace_pie' . $i . '.data = ' . $diskspace[$i] . ';
    ...
    <div id="diskspace_pie' . $i . '" style="width: auto; height: 400px;"></div>
    </div>';
  }
  ?>
