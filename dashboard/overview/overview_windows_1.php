...
foreach ($windows_hostids as $hostID) {
    ...
    $params = array(
        "output" => array("itemid", "lastvalue", "lastclock", "delay"),
        "hostids" => $hostID,
        "search" => array("name" => array("cpu count")),
        "searchByAny" => true
    );
    //call api problem.get only to get eventid
    $result = $zbx->call('item.get',$params);
    ...
    $params = array(
        ...
        "hostids" => $hostID,
        "search" => array("name" => array("Used memory in %")),
        "searchByAny" => true
    );
    //call api problem.get only to get eventid
    $result = $zbx->call('item.get',$params);
...
<html>
    <head></head>
    <body>
        <table id="overview_windows_1_table" class="display" cellspacing="0" width="100%">
            <thead>
                <tr>
                    <th>Host</th>
...
<script>
//table settings
var overview_windows_1_table = $('#overview_windows_1_table').DataTable({
    columnDefs: [
        {
            targets: [ 8 ],
            visible: false,
